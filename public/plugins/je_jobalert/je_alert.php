...
class JE_ALERT {
	...
	function page_template_redirect($page_template){
		 ...
		 * just for testing
		*/
		/*$arg	=	array ( get_term_by( 'slug', 'law', 'job_category') , get_term_by( 'slug', 'architecture', 'job_category'));
		
		$this->mail_alert ( 5 ,$arg );*/
	...
	?>
		<style type="text/css">
			...
			.widget-job-alert .btn-subscribe {
             ...
             /*
			.widget-job-alert .select-style select { width: 100%; }
			.widget-job-alert .select-style span.select { width: 190px; }
               */
	...
	*/
	function je_add_subscriber () {
		...
		if( !et_validate('email', $_REQUEST['email']))  { // check email input is valid
	...
	*/
	...
	function activation () {
		...
		*/
		...
		//run	
		//wp_schedule_event( $time_stamp , $setting['recurring'] , 'et_mail_schedule_event');
		// if(!get_page_by_title( "Unsubscribe" )){
		// 	$unsubscribe_page = array(
		// 		'post_title' 	=> __("Unsubscribe",ET_DOMAIN),
		// 		'post_status'   => 'publish',
		// 		'post_content'  => '',
		// 		'post_type'     => 'page',
		// 		);
	...
	function et_menu_callback ($args) {
		...
		/*$schedule	=	get_option ('je_schedule');
		$job_categories	=	get_option ( 'je_alert_recurren_job_category', array() );*/
		//echo $this->get_alert_message (array(994));
	?>
		<div id="je_alert">
				...
				<div class="et-main-main clearfix" id="subscriber_list" style="display:none;">
		    		...
		    		<div class="module pagination">
		    			<div class="subscriber-controls">
							...
							<?php } ?>
							<!-- <button class="btn-job-export">
			    				<?php _e("Export", ET_DOMAIN); ?>
					...
					<table width="100%" cellspacing="0" cellpadding="0">
						<tr style="background-color: #2e4c6b; display:block; padding:10px 0px; vertical-align: middle; box-shadow: 0 2px 0 2px #E3E3E3;">
							<td align="left" style="padding:10px 5px 10px 20px;width:43%;min-width:300px;display:inline-block">                                              
								<div style="font-weight:bold;font-size:29px;height:35px"><span style="color:white">Will</span><span style="color:#e63a35">works</span></div>
								<div style="color:white;font-size:17px">Job in need are job indeed</div>						
							</td>
							<td align="left" style="padding:10px 10px 10px 0px;width:50%;display:inline-block;text-align:right">
								<a href="[site_url]" target="_blank" style="display: block; position: relative;text-decoration: none; padding: 0; height: 40px; line-height: 40px;">
							...
							<td colspan="3" style="padding: 10px 20px">
								<table>
									<tr>
										...
										SmartJob.vn respectfully send to you the new jobs and hot jobs. Wish you choose is suitable jobs. We honored to accompany success in your career!										
										</td>
									</tr>
									<tr>
											...
											<tr>		
												<td style="text-align: left; padding: 10px 20px;">
													<table>
														...
														<tr>
															<td style="text-align:left;"><a href="[unsubscribe_link]">Unsubscribe</a>
															</td>
														</tr>						
													</table>
												</td>
