...
(function($) {
	...
	if($.fn.jquery.match(/-ajax/)) {
		if('console' in window){ window.console.info('arvfl needs regular jQuery, not the slim version.'); }
		return;
	}
	...
	*/
				...
				$background = $(self.background || [
					...
					'<div class="'+css+'-loading '+css+'">',
						'<div class="'+css+'-content">',
							'<a class="'+css+'-close-icon '+ self.namespace + '-close" aria-label="Close">',
								self.closeIcon,
							'</a>',
		...
		attach: function($source, $content, config) {
			...
			var handler = function(event) {
				...
				var elemConfig = $.extend(
					...
					config);
				var fl = sharedPersist || $target.data('arvfl-persisted') || new Klass($content, elemConfig);
				if(fl.persist === 'shared') {
					sharedPersist = fl;
				} else if(fl.persist !== false) {
					$target.data('arvfl-persisted', fl);
				}
		...
		*/
		_callbackChain: {
			...
			beforeOpen: function(_super, event) {
				...
				$(document.documentElement).addClass('with-arvfl');
			...
			afterClose: function(_super, event) {
				...
				// Restore scroll
				if(arvfl.opened().length === 0) {
					$(document.documentElement).removeClass('with-arvfl');
				}
				return r;
			},
	...
	$.fn.arvfl = function($content, config) {
		arvfl.attach(this, $content, config);
		return this;
	};
